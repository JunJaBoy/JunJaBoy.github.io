...
internal class SmoothCorner(
    ...
    private val angleAlpha = if (shouldCurveInterpolate) toRadians(45.0 * smoothness).toFloat()
    else toRadians(45.0 * smoothness * (1 - interpolationMultiplier)).toFloat()

    // Angle which dictates how much of the curve is going to be a slice of a circle
    private val angleBeta =
        if (shouldCurveInterpolate) toRadians(90.0 * (1.0 - smoothness)).toFloat()
        else toRadians(90.0 * (1 - smoothness * (1 - interpolationMultiplier))).toFloat()
